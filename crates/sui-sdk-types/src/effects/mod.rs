...
impl TransactionEffects {
    ...
    /// transaction — created, mutated, unwrapped, deleted, wrapped,
    /// unwrapped-then-deleted, and packages published or upgraded. V1
    /// effects drain in this order: `created`, `mutated`, `unwrapped`,
    /// `deleted`, `unwrapped_then_deleted`, `wrapped`. V2 effects drain in
    /// the order they appear in `changed_objects`.
    ///
    ...
    ///
    /// V1 effects do not store input digests or input owners, and cannot
    /// reliably distinguish package publishes/upgrades from regular object
    /// creates/mutations (a non-package can also be `Owner::Immutable`).
    /// V1 therefore never emits [`ObjectChange::PackagePublished`] or
    /// [`ObjectChange::PackageUpgraded`], and the optional input fields are
    /// always `None` on V1.
    ///
...
pub enum ObjectChange<'a> {
    ...
    /// A new Move package was published.
    ///
    /// Only emitted from V2 effects. On V1, package publishes appear as
    /// [`ObjectChange::Created`] with `output_owner == Owner::Immutable`.
    PackagePublished {
        object_id: &'a Address,
        version: Version,
        digest: &'a Digest,
    ...
    /// An existing Move package was upgraded.
    ///
    /// Only emitted from V2 effects. On V1, package upgrades appear as
    /// [`ObjectChange::Mutated`] with `output_owner == Owner::Immutable`.
    PackageUpgraded {
        object_id: &'a Address,
        version: Version,
        digest: &'a Digest,
...
impl<'a> ObjectChange<'a> {
    ...
    pub fn object_id(&self) -> &'a Address {
        match self {
            ...
            | Self::UnwrappedThenDeleted { object_id }
            | Self::PackagePublished { object_id, .. }
            | Self::PackageUpgraded { object_id, .. } => object_id,
        }
    }
}
...
fn v2_object_change<'a>(
    ...
    match (
        ...
        (ObjectIn::NotExist, ObjectOut::PackageWrite { version, digest }, IdOperation::Created) => {
            Some(ObjectChange::PackagePublished {
                object_id,
                version: *version,
                digest,
            })
        ...
        (
            ...
            IdOperation::None,
        ) => Some(ObjectChange::PackageUpgraded {
            object_id,
            version: *version,
            digest,
        }),
...
mod tests {
    ...
    fn v1_object_changes_match_fields() {
        ...
        // V1 never emits the V2-only variants.
        assert!(!changes.iter().any(|c| matches!(
            c,
            ObjectChange::PackagePublished { .. } | ObjectChange::PackageUpgraded { .. }
        )));
    }
    ...
    fn object_change_object_id_covers_every_variant() {
        ...
        for change in v1.object_changes().chain(v2.object_changes()) {
            ...
            let id_from_match = match change {
                ...
                | ObjectChange::UnwrappedThenDeleted { object_id }
                | ObjectChange::PackagePublished { object_id, .. }
                | ObjectChange::PackageUpgraded { object_id, .. } => object_id,
            };
