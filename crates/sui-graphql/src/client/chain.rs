...
impl Client {
    ...
    pub async fn chain_identifier(&self) -> Result<Option<String>, Error> {
        ...
        Ok(response.data.and_then(|d| d.chain_identifier))
    }
    ...
    pub async fn protocol_version(&self) -> Result<Option<u64>, Error> {
        ...
        Ok(response.data.and_then(|d| d.protocol_version))
    }
...
mod tests {
    ...
    async fn test_epoch_with_timestamps() {
        ...
        assert_eq!(epoch.epoch_start_timestamp, Some("2024-01-15T00:00:00Z".to_string()));
        assert_eq!(epoch.epoch_end_timestamp, Some("2024-01-16T00:00:00.123Z".to_string()));
    }
}
