...
impl IntentResolver for CoinWithBalanceResolver {
    async fn resolve(
        ...
        for (coin_type, requests) in requests {
            CoinWithBalanceResolver::resolve_coin_type(builder, client, &coin_type, &requests)
                .await?;
        }
    ...
    async fn resolve_coin_type(
            ...
            .as_slice()
        {
            ...
            let amounts = requests
                ...
                .collect(); // = [builder.add_input(pure(&self.balance))];
            builder.split_coins(*first, amounts)
        } else {
